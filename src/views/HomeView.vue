...
export default {
  ...
  methods: {
    getEstudiantes() {
      this.cargando = true;
      axios.get('http://34.16.157.105/api/estudiantes')
        .then(res => {
    ...
    eliminar(id, nombre) {
      confirmar('http://34.16.157.105/api/estudiantes/', id, 'Eliminar registro', 'Realmente desea eliminar a ' + nombre + '?');
    },
    agregarEstudiante() {
      axios.post('http://34.16.157.105/api/estudiantes', this.nuevoEstudiante)
        .then(() => {
